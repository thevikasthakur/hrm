...
router.post('/leavetypes', function(req, res){
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message('Changes to request types were saved');
      return res.redirect_with_session('/settings/general/');
    })
    .catch(error => {
      console.error(
        'An error occurred when trying to edit Request types by user '+req.user.id
        + ' : ' + error
      );
      ...
      req.session.flash_error(
        'Failed to update request types details, please contact customer service'
      );
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    .then(function(company){
        ...
        if (! leave_type_to_remove) {
            ...
            req.session.flash_error('Cannot remove request type: wronge parameters');

            throw new Error(
              'User '+req.user.id+' tried to remove non-existing request type number'
        ...
        // Check if there exist leaves for current type and if so, do not remove it
            ...
            req.session.flash_error('Cannot remove request type: type is in use');

            throw new Error('Failed to remove request type because it is in used.');
        }

        return leave_type_to_remove.destroy();
    })
    .then(function(){
        req.session.flash_message('Request type was successfully removed');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        console.error(
            'An error occurred when trying to remove request type by user' + req.user.id
            + ' : ' + error
        );

        req.session.flash_error(
          'Failed to remove Request Type'
        );
