...
module.exports = function(args){
    ...
    if (!validator.isNumeric(leave_type)){
        req.session.flash_error('Incorrect request type');
    }
    ...
    if (from_date.substr(0,4) !== to_date.substr(0,4)) {
        req.session.flash_error('Current implementation does not allow inter year requests. Please split your request into two parts');
    }
