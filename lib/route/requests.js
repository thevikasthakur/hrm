...
function leave_request_action(args) {
    ...
    .catch(function(error){
      console.error('An error occurred when attempting to '+current_action
        +' a request '+request_id+' by user '+req.user.id+' Error: '+error
      );
      req.session.flash_error('Failed to '+current_action);
      return res.redirect_with_session('../');
    });
...
router.post('/cancel/', function(req, res){
  ...
  .then(function(leaves){
    ...
    if ( ! leave_to_cancel ) {
      throw new Error('Given request is not among those current user can cancel');
    }
  ...
  .then(function(){
    req.session.flash_message('The request was canceled');
  })
  .catch(function(error){
    console.log('An error occurred: '+error);
    req.session.flash_error('Failed to cancel the request');
  })
  .finally(function(){
...
router.post(
  ...
  function(req, res){
    ...
    if (!validator.isNumeric(request_id)){
      req.session.flash_error('Failed to revoke the request');
    }

    if ( req.session.flash_has_errors() ) {
      console.log(
        'Got validation errors when revoking the request for user ' + req.user.id
      );
    ...
    Promise
      ...
      .then(leave_to_process => {
        ...
        if (! leave_to_process) {
          throw new Error('Provided ID '+request_id
            +' does not correspond to any requests to be revoked by user '
            + req.user.id
           );
        }
      ...
      .then(processed_leave => {
        req.session.flash_message(
          'Your request for revocation has been recieved . '
          + (
      ...
      .catch(error => {
        console.error('An error occurred when attempting to revoke the request '
